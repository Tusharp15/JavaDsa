...
class Tester {
    ...
    public static Queue findEvenlyDivisibleNumbers(Queue queue) {
        ...
        while (!queue.isEmpty()) {
            ...
            // Check if the current number is evenly divisible by all numbers from 1 to 10
            if (isEvenlyDivisible(currentNumber)) {
    ...
    private static boolean isEvenlyDivisible(int number) {
        // Check if the number is evenly divisible by all numbers from 1 to 10
        for (int i = 1; i <= 10; i++) {
            if (number % i != 0) {
                return false;
            }
